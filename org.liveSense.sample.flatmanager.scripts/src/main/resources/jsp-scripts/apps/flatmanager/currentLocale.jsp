...

{
	...
	if (pageContext.getAttribute("userNode") != null) {
		...
		if (user.hasProperty("userLocale")) {
			locale = LocaleUtils.toLocale(user.getProperty("userLocale")[0].getString());
		}
	} else {
	...
	if (locale == null) 
		locale = LocaleUtils.toLocale("hu_HU");
	
	pageContext.setAttribute("locale", locale);
	pageContext.setAttribute("resourceBundle", new LocalizationContext(((SlingHttpServletRequest)request).getResourceBundle("org.liveSense.sample.flatmaner.i18n.FlatManagerMessages", locale), locale));
	pageContext.setAttribute("nodeLocale", new JcrNodeWrapper(currentNode, locale));
}
%>
